...
my $tbl_advices = 'mod_advice_advices';
my $tbl_infos = 'mod_advice_infos';
my $tbl_tmp = 'mod_advice_tmp';
...
sub init {
	...
	$dbh->do("INSERT OR REPLACE INTO $tbl_infos (id,name,regex,color) VALUES (1,'Courage Wolf','couragewolf','red')");
	$dbh->do("COMMIT;");
	$advices->{regex}="advice";
	my $sth=$dbh->prepare("SELECT id,name,regex,color FROM $tbl_infos");
...
sub bot_advice {
	...
	if($what=~/$regex$/)
	{
		...
		{
			my $sth=$dbh->prepare("SELECT COUNT(*) FROM $tbl_advices");
			$sth->bind_columns(\$count);
			$sth->execute();
			$sth->fetch();
			$adid=POSIX::floor(rand($count))+1; #1<=Id<=N
		}
		$sth=$dbh->prepare("SELECT typeId,advice,infoId FROM $tbl_advices WHERE id=?");
	...
	else
	{
		foreach $id (sort keys %$advices) 
		{
			...
			if ($id!='regex' and $what=~/$regex$/)
			{
				...
				{
					my $sth=$dbh->prepare("SELECT COUNT(*) FROM $tbl_advices WHERE infoId=?");
					$sth->bind_columns(\$count);
					$sth->execute($id);
					$sth->fetch();
					$adid=POSIX::floor(rand($count))+1; #1<=Id<=N
				}
				$sth=$dbh->prepare("SELECT id,advice,infoId FROM $tbl_advices WHERE infoId=? and typeId=?");
...
sub bot_advice_suggest {
	...
	foreach $id (sort keys %$advices) 
	{
		...
		if ($id!='regex' and $what=~/$regex/)
		{
			my $sth=$dbh->prepare("INSERT INTO $tbl_tmp (user,channel,infoId,advice) VALUES (?,?,?,?)");
...
#	$dbh->do("CREATE TABLE IF NOT EXISTS tmp (id INTEGER PRIMARY KEY, user TEXT, channel TEXT, infoId INTEGER REFERENCES infos (id), advice TEXT UNIQUE)");	
	...
	if($what=~/$regex$/)
	{
		...
		if($what=~/$regex$/)
		{
			my $tmpid=$1;
			#Number to validate
			my $sth=$dbh->prepare("SELECT infoId,advice FROM $tbl_tmp WHERE id=?");
			$sth->bind_columns(\$infoId,\$advice);
			$sth->execute($tmpid);
			if($sth->fetch())
			{
				$sth=$dbh->prepare("SELECT COUNT(*) FROM $tbl_advices WHERE infoId=?");
				$sth->bind_columns(\$typeId);
				$sth->execute($infoId);
				$sth->fetch();
				$typeId++;
				$dbh->do("BEGIN TRANSACTION");
				$sth=$dbh->prepare("INSERT INTO $tbl_advices (typeId,advice,infoId)VALUES(?,?,?)");
				$sth->execute($typeId,$advice,$infoId);
				$sth=$dbh->prepare("DELETE FROM $tbl_tmp WHERE id=?");
				$sth->execute($tmpid);
				$dbh->do("COMMIT");
		...
		else
		{
			...
			my $sth=$dbh->prepare("SELECT id,infoId,advice,user,channel FROM $tbl_tmp");
	...
	else
	{
		foreach $id (sort keys %$advices) 
		{
			...
			if ($id!='regex' and $what=~/$regex$/)
			{
				...
				{
					my $sth=$dbh->prepare("SELECT id,infoId,advice,user,channel FROM $tbl_tmp WHERE infoId=?");
					my ($tmpid,$tmpwho,$tmpwhere);
					$sth->bind_columns(\$tmpid,\$infoId,\$advice,\$tmpwho,\$tmpwhere);
					$sth->execute($id);
					my $count=0;
...
sub bot_advice_stats {
	...
	my $sth=$dbh->prepare("SELECT COUNT(*),infoId FROM $tbl_advices GROUP BY (infoId);");
