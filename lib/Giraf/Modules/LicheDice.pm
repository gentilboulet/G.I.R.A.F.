...
sub init {
	my ($kernel,$irc) = @_;
	Giraf::Trigger::register('public_function','LicheDice','bot_liche_dice',\&bot_liche_dice,'(L|l)iche\s+\d{1,4}(D|d)\d{1,8}');
}
...
sub bot_liche_dice {
	my($nick, $dest, $what)=@_;
	Giraf::Core::debug("bot_liche_dice()");
	$what=~m/^[Ll]iche\s+(\d{1,4})[Dd](\d{1,8})$/;
	my @return;
	my $nbroll=$1;
	my $nbface=$2;
	my @rolls;
